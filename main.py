   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Üzgünüm efendim, yasaklandınız. İletişim [Destek Grubu](https://t.me/destekbotlar).", parse_mode="markdown", disable_web_page_preview=True)
            return 1
      except UserNotParticipant:
         await app.send_message(chat_id=message.chat.id, text="**Lütfen Beni Kullanmak İçin Güncel Kanalıma Katılın!\n ve Kontrol etmek için tıklayın /start**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🤖 Güncelleme Kanalına Katılın 🤖", url=f"https://t.me/dinimbenim")]]), parse_mode="markdown")
         return 1
      except Exception:
         await app.send_message(chat_id=message.chat.id, text="**Bir şeyler ters gitti. İletişim [Destek Grubu](https://t.me/destekbotlar).**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
...
async def to(lel, message):
 try:
   ...
   if a==1:
      return
   '''if message.from_user.id not in PREMIUM:
      await app.send_message(message.chat.id, f"**Artık Premium Kullanıcı Değilsiniz\nLütfen sahibim ile iletişime geçiniz @@slmBATU**")
      return'''
   ...
   To = number.text
   number = await app.ask(chat_id=message.chat.id, text="**Şimdi Üye Çekme işlemine başlayalım Patron**")
   a = int(number.text)
   di=a
   try:
      with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
         ...
         for pphone in str_list:
            ...
            await app.send_message(chat_id=message.chat.id, text=f"**Üyeleri Aktarıyorum Patron Arkana Yaslan...**")
...
async def start(lel, message):
   ...
   if a==1:
      return
   '''if message.from_user.id not in PREMIUM:
      await app.send_message(message.chat.id, f"**Artık Premium Kullanıcı Değilsiniz\nLütfensahibime yazınız @slmBATU**")
      return'''
   try:
      with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
